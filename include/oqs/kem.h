 ...
 *
 * The file `tests/example_kem.c` contains two examples on using the OQS_KEM
 * API.
 *
 ...
 * length member of the OQS_KEM object in question.  Since algorithms can be
 * disabled at compile-time, the programmer should check that the OQS_KEM object
 * is not `NULL`.
 *
...
/**
 * Returns identifiers for available key encapsulation mechanisms in liboqs.
 * Used with OQS_KEM_new.
 *
...
/**
 * Returns the number of key encapsulation mechanisms in liboqs.  They can be
 * enumerated with OQS_KEM_alg_identifier.
 *
...
typedef struct OQS_KEM {
	...
	/** Whether the KEM offers IND-CCA security (TRUE) or IND-CPA security
	 * (FALSE). */
	bool ind_cca;
	 ...
	 * per-scheme compile-time macros `OQS_KEM_*_length_*`.
	 *
	 ...
	 * `shared_secret`, based on the `length_*` members in this object or the
	 * per-scheme compile-time macros `OQS_KEM_*_length_*`.
	 *
	 * @param[out] ciphertext The ciphertext (encapsulation) represented as a byte
	 * string.
	 ...
	 *
	 * @param[out] shared_secret The shared secret represented as a byte string.
	 * @param[in] ciphertext The ciphertext (encapsulation) represented as a byte
	 * string.
 ...
 * @param[in] method_name Name of the desired algorithm; one of the names in
 * `OQS_KEM_algs`.
 * @return An OQS_KEM for the particular algorithm, or `NULL` if the algorithm
 ...
 *
 * Caller is responsible for allocating sufficient memory for `shared_secret`,
 * based on the `length_*` members in this object or the per-scheme compile-time
 * macros `OQS_KEM_*_length_*`.
 *
 * @param[in] kem The OQS_KEM object representing the KEM.
 * @param[out] shared_secret The shared secret represented as a byte string.
 * @param[in] ciphertext The ciphertext (encapsulation) represented as a byte
